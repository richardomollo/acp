...
serve(async (req) => {
  ...
  if (venueWithdrawal) {
    ...
    console.log(`[b2c-callback] venue withdrawal ${venueWithdrawal.id}: ${succeeded ? 'completed' : 'failed'}`)
  ...
  if (ptPayout) {
    ...
    console.log(`[b2c-callback] PT payout ${ptPayout.id}: ${succeeded ? 'completed' : 'failed'}`)
    return new Response('OK', { status: 200 })
  }

  console.warn('[b2c-callback] no withdrawal found for originatorConversationId:', originatorConversationId)
  return new Response('OK', { status: 200 })
})
