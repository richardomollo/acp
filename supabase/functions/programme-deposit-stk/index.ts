...
serve(async (req) => {
  ...
  let body: { phone?: unknown; amount?: unknown; enrollmentId?: unknown; offeringId?: unknown }
  ...
  await admin.from('booking_payments').insert({
    ...
    payment_type: 'full',
    status: 'pending',
    metadata: { source: 'programme-deposit', enrollment_id: enrollmentId, offering_id: offeringId },
  })
