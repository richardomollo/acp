...
type PTStatus = 'pending' | 'approved' | 'rejected' | 'suspended' | null;
...
const STATUS_CONFIG: Record<string, {
  ...
  pending: {
    ...
    message: 'Our team is reviewing your profile. This usually takes 24–48 hours. We\'ll notify you once you\'re approved.',
  },
  approved: {
    icon: 'checkmark-circle', color: '#16a34a', bg: '#f0fdf4',
    title: 'You\'re Approved!',
    message: 'Congratulations! Your trainer profile is live. Go to your dashboard to create offerings and manage bookings.',
  },
  rejected: {
    icon: 'close-circle', color: '#dc2626', bg: '#fef2f2',
    title: 'Application Not Approved',
    message: 'Unfortunately your application wasn\'t approved at this time. See the reason below and feel free to reapply after making updates.',
  },
  suspended: {
    icon: 'ban-outline', color: '#7c3aed', bg: '#f5f3ff',
    title: 'Account Suspended',
    message: 'Your trainer account has been suspended. Please contact support for more information.',
  },
};

export default function PTOnboardingPending() {
  const router = useRouter();
  const [status, setStatus] = useState<PTStatus>(null);
  ...
  const checkStatus = useCallback(async () => {
    try {
      ...
      if (!user) return;
      const [{ data }, { data: gym }] = await Promise.all([
        supabase.from('personal_trainers').select('status, rejection_reason').eq('user_id', user.id).single(),
        supabase.from('gyms').select('id').ilike('contact_email', user.email ?? '').maybeSingle(),
      ]);
      if (data) {
        setStatus(data.status as PTStatus);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <View style={{ width: 40 }} />
        <ThemedText style={styles.headerTitle}>Application Status</ThemedText>
      ...
      <View style={styles.content}>
        ...
        {/* Go to dashboard if approved */}
        {status === 'approved' && (
          <TouchableOpacity
            style={styles.dashboardBtn}
            onPress={() => router.replace('/(pt-tabs)')}
          >
        ...
        {status === 'pending' && (
          <View style={styles.stepsCard}>
            ...
            {[
              { icon: 'mail-outline', text: 'We\'ll email you when your application is reviewed' },
              { icon: 'person-outline', text: 'Your public profile goes live upon approval' },
              { icon: 'calendar-outline', text: 'Set your availability and create session offerings' },
              { icon: 'cash-outline', text: 'Start accepting bookings and earning' },
            ].map((step, i) => (
        ...
        <ThemedText style={styles.supportText}>
          Questions? Email{' '}
          <ThemedText style={styles.supportLink}>support@activecitypass.com</ThemedText>
        </ThemedText>
      </View>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  stepsCard: {
    ...
    padding: 20, marginBottom: 20, borderWidth: 1, borderColor: '#f0f0f0',
  },
  ...
  stepIcon: {
    ...
    backgroundColor: '#f0f5ff', alignItems: 'center', justifyContent: 'center',
  },
  stepText: { flex: 1, fontSize: 14, color: '#444', lineHeight: 20, paddingTop: 7 },
  refreshBtn: {
    flexDirection: 'row', alignItems: 'center', paddingVertical: 12,
    paddingHorizontal: 20, borderRadius: 20, borderWidth: 1,
    borderColor: '#e0e0e0', marginBottom: 20,
  },
