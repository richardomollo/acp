...
const BLANK: {
  name: string; tagline: string; description: string; date: string;
  start_time: string; end_time: string; meeting_point: string; transport_info: string;
  price_kes: string; max_capacity: string; category: string; is_active: boolean;
  image_url: string; gym_id: string;
  includes: string[]; itinerary: ItineraryRow[];
} = {
  name: '', tagline: '', description: '', date: '', start_time: '', end_time: '',
  meeting_point: '', transport_info: '', price_kes: '', max_capacity: '20',
  category: '', is_active: true, image_url: '', gym_id: '',
  includes: [''], itinerary: [{ time: '', activity: '', detail: '' }],
...
export default function ExperiencesScreen() {
  ...
  const load = useCallback(async (isRefresh = false) => {
    try {
      ...
      const gymList = (partnerGyms || []).map((pg: any) => ({
        id: pg.gyms?.id, name: pg.gyms?.name,
      })).filter((g: any) => g.id);

      setGymIds(ids);
      setGyms(gymList);

      if (ids.length === 0) { setExperiences([]); return; }
  ...
  const pickImage = async () => {
    ...
    try {
      ...
      const filename = `experiences/${form.gym_id}/${Date.now()}-${Math.random().toString(36).slice(2)}.${ext}`;
  ...
  const removeInclude = (i: number) => setF('includes', form.includes.filter((_, idx) => idx !== i));
  ...
  const addItinerary = () => setF('itinerary', [...form.itinerary, { time: '', activity: '', detail: '' }]);
  const removeItinerary = (i: number) => setF('itinerary', form.itinerary.filter((_, idx) => idx !== i));
      ...
      {loading ? (
        ...
        <View style={styles.center}>
          ...
          <ThemedText style={styles.emptySub}>
            Create retreats, hikes, and multi-activity events for your members.
          </ThemedText>
      ...
      <Modal visible={showModal} animationType="slide" presentationStyle="pageSheet">
        <View style={styles.modal}>
          ...
          <ScrollView contentContainerStyle={styles.modalForm} keyboardShouldPersistTaps="handled">
            ...
            {/* Venue selector (only if multiple gyms) */}
            {gyms.length > 1 && (
              <MField label="Venue *">
                <View style={styles.venueRow}>
                  {gyms.map(g => (
                    <TouchableOpacity key={g.id}
                      style={[styles.venueChip, form.gym_id === g.id && styles.venueChipActive]}
                      onPress={() => setF('gym_id', g.id)}>
                      <ThemedText style={[styles.venueChipText, form.gym_id === g.id && styles.venueChipTextActive]}>
                        {g.name}
                      </ThemedText>
                    </TouchableOpacity>
                  ))}
            ...
            <MField label="Name *">
              <TextInput style={styles.input} placeholder="e.g. MT Longonot Hike & Yoga Retreat"
            ...
            <ThemedText style={styles.inputHint}>
              Leave at "Venue default" to use your venue's policy, or override for this experience.
            </ThemedText>
            ...
            <View style={styles.inputGroup}>
              ...
              <View style={styles.chipGrid}>
                {DEPOSIT_OPTIONS.map(pct => {
                  ...
                  return (
                    <TouchableOpacity key={String(pct)}
                      style={[styles.policyChip, active && styles.policyChipActive]}
...
const styles = StyleSheet.create({
  ...
  venueChipActive: { backgroundColor: PRIMARY, borderColor: PRIMARY },
  venueChipText: { fontSize: 13, fontWeight: '600', color: '#6b7280' },
