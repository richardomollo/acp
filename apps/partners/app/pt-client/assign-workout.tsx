// Ported from apps/mobile/app/create-workout.tsx (manual builder + AI generator +
// exercise picker) — this monorepo has no shared package between apps (confirmed:
// root package.json workspaces are apps/* only, no packages/ dir), and this
// session's own convention is to duplicate/adapt per-app rather than introduce
// cross-app shared code. The only functional difference from the consumer
// version: handleSave targets a specific client (`user_id: clientId`,
// `assigned_by: <trainer's personal_trainers.id>`) instead of the signed-in
// user's own id, and there is no "edit existing workout" mode — a trainer
// always assigns a fresh workout.
import {
...
const palette = {
  ...
  warning50: '#fffbeb', warning800: '#92400e',
  danger50: '#fef2f2', danger600: '#dc2626',
};
const radii = { sm: 8, md: 12, lg: 16, xl: 20, pill: 100 };
const fontSize = { sm: 13 };

// ── GIF URL helper (same mapping as mobile) ─────────────────────────────────────
...
// ── Workout generator (verbatim port — see apps/mobile/app/create-workout.tsx) ──
...
// Same remount-safety pattern as apps/mobile/create-workout.tsx: force a full
// remount whenever the target client changes, so state from assigning a
// workout to client A can never leak into a session assigning to client B.
export default function AssignWorkoutScreen() {
  const { clientId } = useLocalSearchParams<{ clientId: string }>();
  return <AssignWorkoutForm key={clientId} />;
}
...
function AssignWorkoutForm() {
  ...
  const handleSave = async () => {
    ...
    if (entries.length === 0) {
      Alert.alert('No exercises', 'Add at least one exercise to this workout.');
      return;
    }
    if (!clientId) return;
    ...
    try {
      ...
      if (!user) {
        Alert.alert('Sign in required', 'Please sign in to assign workouts.');
        setSaving(false);
        return;
      }
      ...
      if (!pt) {
        Alert.alert('Error', 'Could not find your trainer profile.');
        setSaving(false);
        return;
      }
      ...
      if (wErr || !workout) {
        Alert.alert('Error', wErr?.message ?? 'Failed to assign workout.');
        setSaving(false);
        return;
      }
      ...
      >
        <View style={s.root}>
          <SafeAreaView edges={['top']} style={s.header}>
            ...
            <ThemedText style={s.headerTitle}>Assign Workout</ThemedText>
            <TouchableOpacity
          ...
          >
            ...
            <View style={s.sectionHeader}>
              <ThemedText style={s.sectionEyebrow}>AI GENERATOR</ThemedText>
              ...
              {GENERATOR_TYPES.map(t => {
                ...
                return (
                  <TouchableOpacity
                    ...
                    activeOpacity={0.75}
                  >
                    <Ionicons name={t.icon as any} size={14} color={active ? '#fff' : palette.gray450} />
            ...
            <View style={s.sectionHeader}>
              <ThemedText style={s.sectionEyebrow}>BUILD</ThemedText>
...
const s = StyleSheet.create({
  ...
  typeGrid: { flexDirection: 'row', gap: 8, paddingRight: 4 },
  typeChip: {
  ...
  typeChipActive: { backgroundColor: palette.ink900, borderColor: palette.ink900 },
  typeLabel: { fontSize: 13, fontWeight: '700', color: palette.ink700 },
