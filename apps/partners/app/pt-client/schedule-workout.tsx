// Ported from apps/mobile/app/schedule-workout.tsx (recurrence + date/time
// picker UI, workout_schedules insert shape) — this monorepo duplicates
// per-app rather than sharing code across apps/*. Differences from the
// client-facing version: targets a specific client (`user_id: clientId`,
// `assigned_by: <trainer's personal_trainers.id>`), adds a Location section,
// and never schedules local notifications itself — those only fire on the
// device that registers them, so the client's own mobile app self-registers
// reminders the next time it loads this schedule (see apps/mobile
// fitness-journey.tsx).
import {
...
const LOCATION_OPTIONS: { key: LocationType; label: string; icon: string }[] = [
...
export default function ScheduleWorkoutScreen() {
  const { workoutId, clientId } = useLocalSearchParams<{ workoutId: string; clientId: string }>();
  return <ScheduleWorkoutForm key={`${workoutId}-${clientId}`} />;
}
...
function ScheduleWorkoutForm() {
  ...
  const handleSchedule = async () => {
    ...
    try {
      ...
      Alert.alert('Scheduled', `${workoutTitle} has been scheduled for your client.`);
      setRecurrence('once');
      setWeekdays([]);
      setLocationAddress('');
      await load();
      ...
      <View style={s.root}>
        ...
        {loading ? (
          ...
          <ScrollView contentContainerStyle={s.content} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
            ...
            <ThemedText style={s.sectionLabel}>Location</ThemedText>
            <View style={s.row}>
            ...
            >
              {saving ? (
                ...
                <>
                  ...
                  <ThemedText style={s.scheduleBtnText}>Schedule</ThemedText>
                </>
              )}
            </TouchableOpacity>
