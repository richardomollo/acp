...
export default function GymProgrammeEnrollScreen() {
  const router = useRouter();
  const { programmeId, enrollmentId } = useLocalSearchParams<{ programmeId: string; enrollmentId: string }>();

  const [programme, setProgramme] = useState<ProgrammeFull | null>(null);
  ...
  const load = async () => {
    const { data } = await supabase
      .from('gym_programmes')
      .select('id, title, gym_id, programme_weeks, programme_price_kes, deposit_pct, instalment_frequency_weeks, gyms!gym_id(id, name, image_url)')
      .eq('id', programmeId)
      .single();

    if (data) {
      const rawGym = Array.isArray((data as any).gyms) ? (data as any).gyms[0] : (data as any).gyms;
      setProgramme({ ...(data as any), gym: rawGym });
    }
    setLoading(false);
  };
  ...
  async function handlePay() {
    ...
    try {
      const payRes = await fetch('https://activecitypass.com/api/mpesa/gym-programme-deposit-stk', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ phone, amount: deposit, enrollmentId, programmeId }),
      });
      const payData = await payRes.json().catch(() => ({}));
      if (!payRes.ok) throw new Error((payData as any)?.error ?? 'Payment initiation failed.');

      const checkoutRequestId = (payData as any)?.checkoutRequestId;
      ...
      const poll = async () => {
        ...
        pollRef.current = setTimeout(async () => {
          try {
            const res = await fetch(`https://activecitypass.com/api/mpesa/status?checkoutRequestId=${encodeURIComponent(checkoutRequestId)}`);
            const data = await res.json().catch(() => ({}));
            const s = String((data as any)?.status ?? '').toUpperCase();
            if (s === 'COMPLETED') {
              await commitEnrollment('paid', (data as any)?.mpesaReceipt ?? checkoutRequestId);
            } else if (['FAILED', 'CANCELLED', 'EXPIRED', 'TIMEOUT'].includes(s)) {
              await commitEnrollment('failed', null);
              setStep('failed');
              setError((data as any)?.reason || 'Payment was not completed. Please try again.');
            } else {
              poll();
            }
          } catch {
  ...
  async function commitEnrollment(paymentStatus: 'paid' | 'failed', mpesaRef: string | null) {
    await fetch('https://activecitypass.com/api/gym-programme', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        action: 'commit',
    ...
    return (
      <View style={styles.centeredContainer}>
        ...
        <ThemedText style={styles.successSub}>
          Your <ThemedText style={{ fontWeight: '800' }}>{programme.title}</ThemedText> journey begins{' '}
          <ThemedText style={{ fontWeight: '700' }}>{fmtDate(startDate!)}</ThemedText>.
        </ThemedText>
        <ThemedText style={styles.successNote}>{programme.gym.name} will be in touch to schedule your sessions.</ThemedText>
  ...
  if (step === 'failed') {
    return (
      <View style={styles.centeredContainer}>
        ...
        <TouchableOpacity style={styles.primaryBtn} onPress={() => { setStep('confirm'); setError(null); }}>
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <ThemedText style={styles.headerSub}>{programme.title}</ThemedText>
        </View>
      </View>
      ...
      >
        <View style={styles.trainerRow}>
          {programme.gym.image_url ? (
            <Image source={{ uri: programme.gym.image_url }} style={styles.trainerAvatar} contentFit="cover" />
          ) : (
            <View style={styles.trainerAvatarFallback}>
              <ThemedText style={styles.trainerInitial}>{programme.gym.name[0].toUpperCase()}</ThemedText>
            </View>
          )}
          <ThemedText style={styles.trainerName}>at {programme.gym.name}</ThemedText>
          <ThemedText style={styles.trainerDuration}>· {programme.programme_weeks} weeks</ThemedText>
        </View>
...
const styles = StyleSheet.create({
  ...
  trainerRow: { flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 20 },
  ...
  scheduleCard: {
    backgroundColor: palette.surfaceMuted, borderRadius: radii.lg, padding: 16,
    marginBottom: 24, borderWidth: 1, borderColor: palette.border,
  },
  scheduleCardTitle: {
  ...
  scheduleTotalRow: {
    ...
    borderTopWidth: 1, borderTopColor: palette.border, marginTop: 4, paddingTop: 12,
  },
  ...
  sectionLabel: { fontSize: fontSize.sm, fontWeight: '700', color: palette.ink900, marginBottom: 10 },

  selectedDateBadge: {
    flexDirection: 'row', alignItems: 'center', gap: 8,
    backgroundColor: '#eef2ff', borderRadius: radii.md,
    paddingHorizontal: 14, paddingVertical: 10, marginBottom: 12, alignSelf: 'flex-start',
  },
  ...
  mpesaInput: {
    ...
    backgroundColor: palette.surfaceMuted, marginBottom: 8,
  },
  ...
  payBtn: { backgroundColor: palette.ink900, paddingVertical: 16, borderRadius: 30, alignItems: 'center', marginBottom: 8 },
  ...
  centeredContainer: { flex: 1, backgroundColor: palette.white, alignItems: 'center', justifyContent: 'center', padding: 32 },
  ...
  failIcon: {
    width: 72, height: 72, borderRadius: 36, backgroundColor: palette.danger500,
    alignItems: 'center', justifyContent: 'center', marginBottom: 20,
  },
  ...
  primaryBtn: { backgroundColor: palette.ink900, paddingVertical: 16, paddingHorizontal: 48, borderRadius: 30, marginTop: 8 },
