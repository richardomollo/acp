...
export default function GymTrainerAssignMealPlanPage({ params }: { params: Promise<{ clientId: string }> }) {
  ...
  const handleSave = async (entries: MealPlanEntry[], name: string) => {
    ...
    const { data: trainer } = await supabase.from("gym_trainers").select("id").eq("user_id", user.id).maybeSingle();
    if (!trainer) { setError("Trainer profile not found."); setSaving(false); return; }

    const { data: plan, error: planErr } = await supabase
      .from("meal_plans")
      .insert({ user_id: clientId, assigned_by_gym_trainer_id: trainer.id, name })
      .select("id")
      .single();
