...
export default function GymProgrammeEnrollClient({
  programme,
  gym,
  enrollmentId,
  programmeId,
  ...
  const weeks = programme.programme_weeks;
  ...
  async function handlePay() {
    ...
    try {
      const payRes = await fetch("/api/mpesa/gym-programme-deposit-stk", {
        method: "POST",
    ...
    return (
      <div className="max-w-lg mx-auto py-16 text-center">
        ...
        <p className="text-gray-500 text-sm mb-1">
          Your <strong>{programme.title}</strong> journey begins{" "}
          <strong>{new Date(startDate + "T00:00:00").toLocaleDateString("en-KE", { weekday: "long", day: "numeric", month: "long" })}</strong>.
        </p>
        <p className="text-gray-400 text-xs mb-8">{gym.name} will be in touch to schedule your sessions.</p>
        ...
        <Link href={`/venues/${gym.id}`} className="inline-block bg-black text-white text-sm font-semibold px-8 py-3 rounded-full hover:bg-gray-800 transition-colors">
          View venue
        </Link>
      </div>
    );
  ...
  return (
    <div className="max-w-lg mx-auto">
      <div className="mb-8">
        ...
        <p className="text-sm text-gray-500 mt-1">at {gym.name} · {programme.programme_weeks} weeks</p>
      </div>
      ...
      <div className="mb-5">
        ...
        />
        <p className="text-xs text-gray-400 mt-1.5">Sessions will be scheduled directly with {gym.name} after this date.</p>
      </div>
