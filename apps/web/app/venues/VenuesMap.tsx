...
type Props = { gyms: Gym[]; activeId: string | null; onSelect: (id: string) => void };
  ...
  return (
    <>
      ...
      {activeGym && activePos && (
        <InfoWindow
          position={activePos}
          onCloseClick={() => onSelect(activeId!)}
          pixelOffset={[0, -42]}
        >
          <div style={{ fontFamily: "inherit", width: 210 }}>
            {activeGym.image_url && (
            ...
            <p style={{ fontSize: 11, color: "#888", textTransform: "capitalize", marginBottom: 2 }}>{activeGym.type}</p>
            <p style={{ fontWeight: 700, fontSize: 14, marginBottom: 3, lineHeight: 1.3 }}>{activeGym.name}</p>
            <p style={{ fontSize: 12, color: "#666", marginBottom: 4 }}>{activeGym.location}{activeGym.area ? `, ${activeGym.area}` : ""}</p>
            {activeGym.rating > 0 && <p style={{ fontSize: 12, marginBottom: 6 }}><span style={{ color: "#f59e0b" }}>★</span> {activeGym.rating}</p>}
