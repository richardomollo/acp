...
function getCoords(area: string): [number, number] | null {
  return AREA_COORDS[area?.trim()] ?? null;
}

function FlyToActive({ activeId, gyms }: { activeId: string | null; gyms: MapGym[] }) {
  const map = useMap();
  useEffect(() => {
    if (!activeId) return;
        ...
        const icon = L.divIcon({
          html: `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size * 1.4}" viewBox="0 0 28 40">
            <path d="M14 0C6.268 0 0 6.268 0 14c0 10.5 14 26 14 26S28 24.5 28 14C28 6.268 21.732 0 14 0z"
              fill="${isActive ? "#000" : "#111"}" opacity="${isActive ? "1" : "0.75"}"/>
              ...
              <div style={{ fontFamily: "inherit" }}>
                {gym.image_url && (
                  <img
                    src={gym.image_url}
                    alt={gym.name}
                    style={{ width: "100%", height: 110, objectFit: "cover", display: "block", borderRadius: "6px 6px 0 0", marginBottom: 10 }}
                  />
                )}
                <div style={{ padding: "0 4px 4px" }}>
                  ...
                  <p style={{ fontWeight: 700, fontSize: 14, marginBottom: 4 }}>{gym.name}</p>
                  <p style={{ fontSize: 12, color: "#666", marginBottom: 6 }}>{gym.location}</p>
                  {gym.sessionCount != null && (
                    <p style={{ fontSize: 12, color: "#555", marginBottom: 8 }}>
                      {gym.sessionCount} class{gym.sessionCount !== 1 ? "es" : ""} available
                    </p>
                  )}
                  <a href={`/venues/${gym.id}`} style={{ fontSize: 13, color: "#050040", fontWeight: 600, textDecoration: "none" }}>
                    View venue →
                  </a>
                </div>
              </div>
